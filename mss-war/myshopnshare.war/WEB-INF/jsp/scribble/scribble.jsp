...
<%@ include file="../scribble/form.jsp" %>
				...
				<div id="all-scribbles" class="postings hide-element">
						...
						<ul id="scribble-content" class="item-journals">
							<c:forEach items="${person.activeScribbles}" var="scribble">
								<sns:ownscribbles primaryKey="${scribble.id}" thumbnail="${scribble.scribbler.defaultFace.icon.path}" fullName="${scribble.scribbler.fullName}" message="${scribble.message}" enteredDate="${scribble.enteredDate}" ></sns:ownscribbles>
							</c:forEach>
							<h3>Your Scribbles On Other's Wall</h3>
							<c:forEach items="${person.activeScribbledOns}" var="scribble">
								<sns:ownscribbles primaryKey="${scribble.id}" thumbnail="${scribble.scribbler.defaultFace.icon.path}" fullName="${scribble.scribbler.fullName}" message="${scribble.message}" enteredDate="${scribble.enteredDate}" ></sns:ownscribbles>
							</c:forEach>													
						</ul>
				</div>            
