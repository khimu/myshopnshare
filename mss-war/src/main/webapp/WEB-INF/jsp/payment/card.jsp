...
$(document).ready(function() { 		
	$('#submit-creditcard-button').click(function(){
		...
		var email_update = jQuery("#add-creditcard-form").validate( {
			...
			messages : {
				fullname :"<div class='errors'>Please enter your full name type</div>",
				number :"<div class='errors'>Please enter a valid ccard number address</div>",
				expirationDate :"<div class='errors'>Please enter the card expiration date</div>",
				securityCode :"<div class='errors'>Please enter the card security code</div>",
				creditCardType :"<div class='errors'>Please select a credit card type</div>"
			},
			submitHandler : function(form) {
				jQuery(form).ajaxSubmit( {
					...
					success: function(c) {
						$('#card-element').hide();
						$('#card-selection').append('<option value="' + c.id + '">'+c.lastFour+'</option>');
	...
	<form id="add-creditcard-form">
		...
		<div>
			...
			<div><label for="creditCardType">Card Type</label></div>
			<div><span class="asterisk">*</span><select name="creditCardType">
