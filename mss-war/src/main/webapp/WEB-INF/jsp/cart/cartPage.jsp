...
<div id="shopping-cart-element">
	...
	<c:set var="grandTotal">
		<fmt:formatNumber value="${cartItemsGrandTotal}" pattern="#,###,###.00" />
	</c:set>		
	...
	<sns:report tableName="cartTable" columns="Item Name|Price|Shipping|Tax|Quantity|Total">
	<c:forEach items="${rewardsItems}" var="rewardsItem">
	...
	<c:set var="grandTotal">
		<fmt:formatNumber value="${rewardsItemsGrandTotal}" pattern="#,###,###.00" />
	</c:set>	
	<sns:carttotal records="${grandTotal}" idType="cart" primaryKey="${rewardsItem.id}" />
	</sns:report>
	
	<div style="padding: 10px;">	
	<c:if test="${not empty cartItems or not empty rewardsItems}">
